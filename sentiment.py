...
def visualize(df):
    ...
    def bernoulli_nb_classifier(X_train, y_train, X_test, vectorizer):
        ...
        for i, c in enumerate(unique_classes):
            ...
            class_word_counts = X_train_dense[class_indices].sum(axis=0)
        ...
        log_prior_probs = np.log(prior_probs)
        log_likelihood_probs = np.log(likelihood_probs)
    
        # Initialize array to store predictions
        y_pred = np.zeros(X_test_dense.shape[0])
    
        # Make predictions for each document in the test set
        for i in range(X_test_dense.shape[0]):
            document_vector = X_test_dense[i, :]
            log_probs = np.zeros(len(unique_classes))
    
            for j, c in enumerate(unique_classes):
                # Calculate log-likelihood for the current class
                log_probs[j] = log_prior_probs[j] + np.sum(document_vector * log_likelihood_probs[j, :])
    
            # Assign the class with the highest log probability as the predicted class
            y_pred[i] = unique_classes[np.argmax(log_probs)]
